...
#ref https://docs.okd.io/latest/creating_images/guidelines.html#openshift-specific-guidelines
mkdir -p /var/lib/kibana
for dir in "${HOME}" "${KIBANA_HOME}" "${KIBANA_CONF_DIR}" "/var/lib/kibana" ; do
  chgrp -R 0 $dir
  chmod -R g=u $dir
  chmod -R oga+rw $dir
done
